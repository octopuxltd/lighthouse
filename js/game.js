...
const rooms = {
    "0,0": {
        name: "Spiral stair",
        description: "The iron spiral stair coils up through the heart of the tower, its treads worn shallow by decades of boots. Cold air falls from the lamp room above and the kitchen lies a few steps down.",
        blocked: {
    ...
    "1,0": {
        name: "Lamp room",
        description: "Glass panes wrap the lamp room on every side and the great lens sits dark and patient at its centre. Far below, the sea works endlessly against the rocks.",
        blocked: {
    ...
    "0,1": {
        ...
        description: "A cold cast-iron stove and a single chair furnish the keeper's kitchen, and a mug of tea has long gone to scum on the table. A narrow doorway opens onto the foot of the stair.",
        blocked: {
    ...
    "1,1": {
        name: "The rocks",
        description: "You stand on the black, weed-slick rocks at the foot of the lighthouse as spray bursts around your knees. The tower door hangs open to the north and a low path skirts the base to the west.",
        blocked: {
...
function spiralStairArt() {
    ...
    return sketch("A sketch of an iron spiral staircase", 
    // central newel post + knob, the treads, and a base ring
    `<line x1="108" y1="24" x2="108" y2="128"/>` +
        `<circle cx="108" cy="22" r="3"/>` +
        treads +
        `<path d="M92 128 Q108 134 124 128" stroke="${FAINT}"/>`, 4);
}
const art = {
    // Spiral stair (top left)
    "0,0": spiralStairArt(),
    // Lamp room (top right): a fresnel-lens drum throwing beams out through glass.
    "1,0": sketch("A sketch of the lamp room lens throwing out beams of light", `<line x1="55" y1="124" x2="165" y2="124"/>` +
        ...
        `<path d="M92 92 L28 114" stroke="${GLOW}"/>`, 7),
    // Keeper's kitchen (bottom left): stove with a pot, a steaming mug on a table,
    // a plain chair.
    "0,1": sketch("A sketch of the keeper's kitchen with a stove, a mug and a chair", `<line x1="20" y1="126" x2="200" y2="126"/>` +
        // stove
        ...
        `<line x1="180" y1="84" x2="176" y2="84"/>`, 11),
    // The rocks (bottom right): the lighthouse on the rocks, light lit, waves below.
    "1,1": sketch("A sketch of the lighthouse standing on the rocks above the waves", `<path d="M20 132 q9 -7 18 0 t18 0 t18 0 t18 0 t18 0 t18 0 t18 0 t18 0 t18 0" stroke="${FAINT}"/>` +
        ...
        `<path d="M103 34 L70 26" stroke="${GLOW}"/>`, 2),
};
const palettes = {
...
function availableDirections() {
    ...
    return Object.keys(deltas).filter((dir) => {
        if (room.blocked[dir])
            return false;
...
function move(dir) {
    ...
    if (moving)
        return; // ignore keys mid-fade so transitions don't stack
    moving = true;
